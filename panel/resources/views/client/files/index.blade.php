...
<div class="space-y-4">
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('client.sites.index') }}" class="text-gray-500 hover:text-white transition">
        ...
        <div>
            <h1 class="text-xl font-black tracking-tight flex items-center gap-2">
                ...
                Archivos — <span class="text-indigo-400 font-mono">{{ $site->domain }}</span>
            </h1>
        </div>
    </div>
    ...
    <div class="flex rounded-2xl border border-white/10 overflow-hidden"
        ...
        <div class="w-64 shrink-0 border-r border-white/10 flex flex-col bg-[#0d0e11]">
            ...
            {{-- Breadcrumb --}}
            <div id="fm-breadcrumb" class="px-3 py-2 border-b border-white/10 text-xs text-gray-500 truncate font-mono">
                /
            </div>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <div class="border-b border-white/10 px-4 py-2 flex items-center justify-between bg-[#0d0e11]">
                ...
                <div class="flex items-center gap-2">
                    <button id="btn-download" onclick="FM.downloadCurrent()" title="Descargar"
                        class="hidden px-3 py-1.5 rounded-lg text-xs bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white transition">
                        Descargar
                    </button>
                    <button id="btn-save" onclick="FM.save()"
                        class="hidden px-4 py-1.5 rounded-lg text-xs font-semibold bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 hover:text-indigo-100 transition">
                        Guardar
                    </button>
                </div>
            </div>

            {{-- Monaco / Image Preview --}}
...
function isImage(name) { return /^(png|jpg|jpeg|gif|svg|webp|ico|bmp)$/i.test(ext(name)); }

function langFor(name) {
    const map = { php:'php', js:'javascript', ts:'typescript', jsx:'javascript', tsx:'typescript',
                  html:'html', htm:'html', css:'css', scss:'css', json:'json',
                  yml:'yaml', yaml:'yaml', py:'python', sh:'shell', bash:'shell',
                  md:'markdown', xml:'xml', sql:'sql', txt:'plaintext',
                  env:'plaintext', gitignore:'plaintext', htaccess:'ini' };
    return map[ext(name)] || 'plaintext';
}

function fileIcon(entry) {
    if (entry.is_dir) return '📁';
    const e = ext(entry.name);
    if (isImage(entry.name)) return '🖼️';
    const icons = { php:'🐘', js:'📜', ts:'📘', html:'🌐', css:'🎨', json:'📋',
                    md:'📝', sql:'🗄️', sh:'⚙️', py:'🐍', env:'🔧', xml:'📄' };
    return icons[e] || '📄';
}

function showToast(msg, type = 'success') {
    const el = document.createElement('div');
    el.className = `fixed bottom-6 right-6 z-[100] px-5 py-3 rounded-2xl text-sm font-semibold shadow-2xl transition-all duration-300 ${
        type === 'error' ? 'bg-red-500/20 border border-red-500/30 text-red-200' : 'bg-emerald-500/20 border border-emerald-500/30 text-emerald-200'
    }`;
    el.textContent = msg;
...
async function loadDirectory(path) {
    ...
    document.getElementById('fm-breadcrumb').textContent = state.currentPath;

    const inner = document.getElementById('file-list-inner');
    inner.innerHTML = '<div class="px-4 py-8 text-center text-gray-600 text-xs">Cargando...</div>';

    try {
        const data = await api('GET', `/list?domain=${encodeURIComponent(DOMAIN)}&path=${encodeURIComponent(path)}`);
        renderFileList(data.entries || []);
        renderTree(path);
    } catch (e) {
        inner.innerHTML = `<div class="px-4 py-8 text-center text-red-400 text-xs">Error: ${e.message}</div>`;
    }
}
...
function renderFileList(entries) {
    const inner = document.getElementById('file-list-inner');
    if (!entries.length) {
        inner.innerHTML = '<div class="px-4 py-8 text-center text-gray-600 text-xs">Carpeta vacía</div>';
        return;
    }
    inner.innerHTML = entries.map(e => `
        <div class="flex items-center gap-2.5 px-3 py-2 cursor-pointer hover:bg-white/5 rounded-lg mx-1 group transition select-none
                    ${state.selectedEntry?.path === e.path ? 'bg-white/5 text-white' : 'text-gray-400'}"
             data-path="${e.path}" data-isdir="${e.is_dir}" data-name="${e.name}"
             onclick="FM.selectEntry(this, ${JSON.stringify(e).replace(/"/g, '&quot;')})"
             ondblclick="FM.openEntry(${JSON.stringify(e).replace(/"/g, '&quot;')})"
             oncontextmenu="FM.showCtxMenu(event, ${JSON.stringify(e).replace(/"/g, '&quot;')})">
            <span class="text-base leading-none">${fileIcon(e)}</span>
            <span class="flex-1 min-w-0 text-xs truncate ${e.is_dir ? 'font-semibold text-white/80' : ''}">${e.name}</span>
            ${!e.is_dir ? `<span class="text-[10px] text-gray-600 shrink-0">${formatSize(e.size)}</span>` : ''}
        </div>
    `).join('');
}
...
function renderTree(activePath) {
    ...
    paths.forEach((p, depth) => {
        const name = depth === 0 ? '/' : p.split('/').pop();
        html += `<div class="flex items-center gap-1.5 px-3 py-1.5 cursor-pointer text-xs rounded-lg mx-1 transition select-none
                      ${p === activePath ? 'bg-white/5 text-white font-semibold' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5'}"
                      style="padding-left:${12 + (depth * 12)}px"
                      onclick="loadDirectory('${p}')">
                 <span>📂</span>${name}</div>`;
    });
    root.innerHTML = html;
}
...
// ==================== OPEN / SELECT ====================
const FM = {
    selectEntry(el, entry) {
        document.querySelectorAll('[data-path]').forEach(e => e.classList.remove('bg-white/5', 'text-white'));
    ...
    async openFile(entry) {
        ...
        document.getElementById('editor-filename').textContent = entry.name;
        ...
        if (isImage(entry.name)) {
            ...
            document.getElementById('preview-img').src = BASE + `/download?domain=${encodeURIComponent(DOMAIN)}&path=${encodeURIComponent(entry.path)}`;
            return;
        }
        ...
        try {
            const data = await api('GET', `/read?domain=${encodeURIComponent(DOMAIN)}&path=${encodeURIComponent(entry.path)}`);
            if (state.editor) {
                const model = monaco.editor.createModel(data.content || '', langFor(entry.name));
    ...
    async save() {
        ...
        try {
            await api('POST', '/write', { domain: DOMAIN, path: state.openPath, content });
            state.isDirty = false;
            document.getElementById('editor-dirty').classList.add('hidden');
    ...
    downloadCurrent() {
        ...
        window.open(BASE + `/download?domain=${encodeURIComponent(DOMAIN)}&path=${encodeURIComponent(state.openPath)}`);
    },

    // ==================== TOOLBAR ACTIONS ====================
    newFile() {
        showInputModal('Nombre del nuevo archivo', 'archivo.txt', async (name) => {
            const path = state.currentPath.replace(/\/$/, '') + '/' + name;
            try {
                await api('POST', '/write', { domain: DOMAIN, path, content: '' });
    ...
    newFolder() {
        showInputModal('Nombre de la nueva carpeta', 'nueva-carpeta', async (name) => {
            const path = state.currentPath.replace(/\/$/, '') + '/' + name;
            try {
                await api('POST', '/mkdir', { domain: DOMAIN, path });
                await loadDirectory(state.currentPath);
    ...
    async uploadFiles(files) {
        ...
        for (const file of files) {
            ...
            try {
                ...
                prog.style.width = Math.round((done / files.length) * 100) + '%';
            } catch(e) {
                showToast('Error al subir ' + file.name + ': ' + e.message, 'error');
            }
        }
        setTimeout(() => { bar.classList.add('hidden'); prog.style.width = '0%'; }, 1000);
        await loadDirectory(state.currentPath);
        showToast(`${done} archivo(s) subido(s)`);
        document.getElementById('upload-input').value = '';
    },

    handleDrop(event) {
        event.preventDefault();
        event.currentTarget.classList.remove('bg-indigo-500/5');
        const files = Array.from(event.dataTransfer.files);
        if (files.length) this.uploadFiles(files);
    },

    // ==================== CONTEXT MENU ====================
    showCtxMenu(event, entry) {
        event.preventDefault();
    ...
    ctxOpen() {
        ...
        if (state.ctxEntry) this.openEntry(state.ctxEntry);
    },
    ...
    ctxRename() {
        ...
        if (!entry) return;
        showInputModal('Renombrar', entry.name, async (newName) => {
            const dir = entry.path.substring(0, entry.path.lastIndexOf('/') + 1);
            const newPath = dir + newName;
            try {
                await api('POST', '/rename', { domain: DOMAIN, old_path: entry.path, new_path: newPath });
                await loadDirectory(state.currentPath);
    ...
    ctxDownload() {
        document.getElementById('ctx-menu').classList.add('hidden');
        if (state.ctxEntry && !state.ctxEntry.is_dir) {
            window.open(BASE + `/download?domain=${encodeURIComponent(DOMAIN)}&path=${encodeURIComponent(state.ctxEntry.path)}`);
        }
    },
    ...
    async ctxDelete() {
        ...
        const entry = state.ctxEntry;
        if (!entry) return;
        if (!confirm(`¿Eliminar "${entry.name}"? Esta acción no se puede deshacer.`)) return;
        try {
            await api('POST', '/delete', { domain: DOMAIN, path: entry.path });
            await loadDirectory(state.currentPath);
    ...
    inputConfirm() {
        ...
        if (val && state.inputCallback) state.inputCallback(val);
...
require(['vs/editor/editor.main'], function() {
    ...
    // Sync Monaco theme with dark mode toggle
