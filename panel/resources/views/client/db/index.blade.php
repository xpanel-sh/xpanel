...
@section('content')
    <div
        ...
        <div class="flex flex-col grow kt-scrollable-y lg:[scrollbar-width:auto] pt-7 lg:[&amp;_.kt-container-fluid]:pe-4"
                    ...
                    <div class="grid gap-5 lg:gap-7.5">
                        <div class="space-y-6" x-data="{ search: '{{ request('search') }}', showConn: false }">
                            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                                <div>
                                    <p class="text-sm font-semibold uppercase tracking-[0.25em] text-gray-500">Panel Cliente
                                    </p>
                            ...
                            <div class="rounded-2xl border border-blue-500/20 bg-blue-500/5 p-4">
                                <button @click="showConn = !showConn"
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm font-semibold text-blue-300">Datos de conexión para tu código
                                            PHP</span>
                                    </div>
                                    <svg class="w-4 h-4 text-blue-400 transition-transform"
                                        :class="showConn ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M19 9l-7 7-7-7" />
                                    </svg>
                                ...
                                <div x-show="showConn" x-collapse class="mt-4 space-y-3">
                                    ...
                                    <div class="rounded-xl bg-gray-900 p-3">
                                        ...
                                        <code class="text-xs text-green-400 font-mono whitespace-pre">$pdo = new
                                            PDO('mysql:host=localhost;port=3306;dbname=TUBD', 'TUUSER', 'TUPASS');</code>
                                    </div>
                                    <p class="text-xs text-gray-500">
                                        XPanel enruta automáticamente <code class="text-gray-400">localhost:3306</code> a la
                                        base de datos — usa siempre <code class="text-gray-400">localhost</code> en tu
                                        código PHP.
                                    </p>
                                </div>
                            </div>
                            ...
                            <form method="GET" action="{{ route('client.databases.index') }}" class="flex gap-3">
                                ...
                                <button type="submit"
                                    class="rounded-xl border border-white/10 px-5 py-2.5 text-sm text-gray-300 hover:bg-white/5 transition">Buscar</button>
                                @if (request('search'))
                                    <a href="{{ route('client.databases.index') }}"
                                        class="rounded-xl border border-white/10 px-4 py-2.5 text-sm text-gray-400 hover:bg-white/5 transition">✕
                                        Limpiar</a>
                                @endif
                            </form>
                                        ...
                                        <tbody class="divide-y divide-white/10">
                                            @forelse($databases as $database)
                                                <tr class="hover:bg-white/[0.02] transition">
                                                    <td class="px-6 py-4 font-semibold text-white">{{ $database->name }}
                                                    </td>
                                                    <td class="px-6 py-4 text-gray-400">{{ $database->username }}</td>
                                                    <td class="px-6 py-4 text-gray-400 uppercase text-xs font-bold">
                                                        {{ $database->engine }}</td>
                                                    <td class="px-6 py-4 text-gray-500 text-sm">
                                                        {{ $database->site?->domain ?? 'Sin sitio' }}</td>
                                                    <td class="px-6 py-4">
                                                        @php
                                                        ...
                                                        @endphp
                                                        <span
                                                            class="rounded-full border px-3 py-1 text-xs font-bold uppercase {{ $statusClass }}">{{ $database->status }}</span>
                                                    </td>
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                                        @if (request('search'))
                                                            No se encontraron bases de datos que coincidan con
                                                            "{{ request('search') }}".
                                                        @else
