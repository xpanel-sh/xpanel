...
@section('content')
    <section class="space-y-6">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <a href="{{ route('client.domains.create') }}" class="rounded-xl bg-white px-5 py-3 text-sm font-bold text-black transition hover:bg-gray-200">
                Agregar dominio
            </a>
        </div>
                        ...
                        @forelse($domains as $domain)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <form action="{{ route('client.domains.destroy', $domain) }}" method="POST" onsubmit="return confirm('¿Eliminar dominio?')" class="inline">
                                        @csrf
                                        @method('DELETE')
